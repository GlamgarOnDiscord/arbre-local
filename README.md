...
**Lieu de décès désormais résolu.** Le fichier fournit `code_insee_deces` sans libellé ; le site le croise maintenant avec le Code Officiel Géographique de l'INSEE pour afficher un nom de commune (voir plus haut). Les codes de communes fusionnées avant 1943 ou mal renseignés restent parfois non résolus.
...
- ~~Résolution des codes INSEE via le Code Officiel Géographique~~ — fait (voir plus haut)
- **Export GEDCOM** de l'arbre local pour réimporter dans Gramps ou Ancestris
